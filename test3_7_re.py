...
#    pattern = re.compile('<div.*?author clearfix">.*?<h2>(.*?)</h2></a><div class="articleGender .*?">(.*?)</div>.*?<a.*?class='contentHerf'><div class="content"><span>(.*?)</span></div></a>(.*?)<div class="stats"><span class="stats-vote"><i class="number">(.*?)</span>',re.S)
